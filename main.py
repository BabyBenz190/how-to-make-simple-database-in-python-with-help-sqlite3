...
def log_in():
    ...
    try:
       ...
       cursor.execute("SELECT login FROM users WHERE login = ?", [login])
       if cursor.fetchone() is None:#если такого логина не существует в нашей таблице
        ...
        #если же логин  существует,то делаем проверку на пароль.
            ...
            else:#если пароль верный,то перекидываем пользователя в функцию roulette.
                play_roulette(login)
    except sqlite3.Error as e:
        print("Error", e)
    finally:
...
def play_roulette(login):
    ...
    try:
        ...
        if cursor.fetchone() is None:
            ...
            bet = int(input("Bet: "))#ставка в рулетке
            number = random.randint(1, 100)#ставим простой диапазон

            balance = cursor.execute("SELECT balance FROM users WHERE login = ?", [login]).fetchone()[0]
            if balance < bet:#если балланс меньше ставки
                print("Поднимай жопу из-за компьютера и иди работай,а не сливай кэш в рулетку:)")
            elif balance <=0:#если балланс отрицательный или равен 0,то пользователь также не сможет играть в рулетку.
                print("Поднимай жопу из-за компьютера и иди работай,а не сливай кэш в рулетку:)")
            else:
                if number < 50:#если в переменной "number"  получилось число меньше 50,то пользователь проигрывает и деньги переводятся на счет рулетки.
